...
#include "nl-logo.h" // This contains the obstacle bitmaps
...
#define BG_GREEN  0 // was 20
#define BG_BLUE   0 //was 80
...
int main(int argc, char **argv) {
	...
	int x1 = (width  - NL_WIDTH ) / 2;
	int y1 = (height - NL_HEIGHT) / 2;
	uint8_t pixel[3];
	char *ptr=header_data;
	for(y=0; y<LOGO_HEIGHT; y++) {
		for(x=0; x<LOGO_WIDTH; x++) {
		  HEADER_PIXEL(ptr,pixel);
		  if (pixel[0]>=50) {
	...
	while(running) {
		...
		for(y=0; y<NL_HEIGHT; y++) {
			for(x=0; x<NL_WIDTH; x++) {
			  ...
			  /*
			  if (pixel[0]>=50) {
			    led_canvas_set_pixel(canvas,x1+x,y1+y,200,200,200);
			    }*/
			  led_canvas_set_pixel(canvas, x1+x, y1+y,pixel[0],pixel[1],pixel[2]);
			}
		}
