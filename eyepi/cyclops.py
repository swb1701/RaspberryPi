...
#start of added section for socket comms -- SWB

import pickle #swb
import selectors #swb
import socket #swb
import types #swb

host='127.0.0.1'
port=65432 #might as well use what PaulZC did (not sure I'll send same data yet though)

shared={"curX":-1,"curY":-1,"pupil":-1,"nolid":-1} #negative one means no override
...
#end of added section for socket comms -- SWB
...
#start of methods for socket comms -- SWB
...
#end of methods for socket comms -- SWB    
...
#		print(frames/(now-beginningTime))
	...
	if shared['curX']!=-1 or shared['curY']!=-1: #JOYSTICK_X_IN >= 0 and JOYSTICK_Y_IN >= 0:
		# Eye position from analog inputs
		curX = shared['curX']#bonnet.channel[JOYSTICK_X_IN].value
		curY = shared['curY']#bonnet.channel[JOYSTICK_Y_IN].value
		curX = -30.0 + curX * 60.0
		curY = -30.0 + curY * 60.0
	else :
